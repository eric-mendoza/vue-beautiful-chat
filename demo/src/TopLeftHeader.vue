<template>
  <div class="top-left-header">
    <div class="top-left-header--text">
      <img class="profile--img" :src="profileImage" alt="Profile image" @click="openProfile" />
    </div>
    <div class="top-left-header--tools">
      <div class="tool-button" @click="showNewContactMenu">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20"><path class="heroicon-ui" d="M19 10h2a1 1 0 0 1 0 2h-2v2a1 1 0 0 1-2 0v-2h-2a1 1 0 0 1 0-2h2V8a1 1 0 0 1 2 0v2zM9 12A5 5 0 1 1 9 2a5 5 0 0 1 0 10zm0-2a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm8 11a1 1 0 0 1-2 0v-2a3 3 0 0 0-3-3H7a3 3 0 0 0-3 3v2a1 1 0 0 1-2 0v-2a5 5 0 0 1 5-5h5a5 5 0 0 1 5 5v2z"/></svg>
      </div>
      <div class="tool-button" @click="showNewGroupMenu">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20"><path class="heroicon-ui" d="M9 12A5 5 0 1 1 9 2a5 5 0 0 1 0 10zm0-2a3 3 0 1 0 0-6 3 3 0 0 0 0 6zm7 11a1 1 0 0 1-2 0v-2a3 3 0 0 0-3-3H7a3 3 0 0 0-3 3v2a1 1 0 0 1-2 0v-2a5 5 0 0 1 5-5h4a5 5 0 0 1 5 5v2zm1-5a1 1 0 0 1 0-2 5 5 0 0 1 5 5v2a1 1 0 0 1-2 0v-2a3 3 0 0 0-3-3zm-2-4a1 1 0 0 1 0-2 3 3 0 0 0 0-6 1 1 0 0 1 0-2 5 5 0 0 1 0 10z"/></svg>
      </div>
      <div class="tool-button" @click="showContacts">
        &plus;
      </div>
      <div class="tool-button" @click="showSettings">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20"><path class="heroicon-ui" d="M9 4.58V4c0-1.1.9-2 2-2h2a2 2 0 0 1 2 2v.58a8 8 0 0 1 1.92 1.11l.5-.29a2 2 0 0 1 2.74.73l1 1.74a2 2 0 0 1-.73 2.73l-.5.29a8.06 8.06 0 0 1 0 2.22l.5.3a2 2 0 0 1 .73 2.72l-1 1.74a2 2 0 0 1-2.73.73l-.5-.3A8 8 0 0 1 15 19.43V20a2 2 0 0 1-2 2h-2a2 2 0 0 1-2-2v-.58a8 8 0 0 1-1.92-1.11l-.5.29a2 2 0 0 1-2.74-.73l-1-1.74a2 2 0 0 1 .73-2.73l.5-.29a8.06 8.06 0 0 1 0-2.22l-.5-.3a2 2 0 0 1-.73-2.72l1-1.74a2 2 0 0 1 2.73-.73l.5.3A8 8 0 0 1 9 4.57zM7.88 7.64l-.54.51-1.77-1.02-1 1.74 1.76 1.01-.17.73a6.02 6.02 0 0 0 0 2.78l.17.73-1.76 1.01 1 1.74 1.77-1.02.54.51a6 6 0 0 0 2.4 1.4l.72.2V20h2v-2.04l.71-.2a6 6 0 0 0 2.41-1.4l.54-.51 1.77 1.02 1-1.74-1.76-1.01.17-.73a6.02 6.02 0 0 0 0-2.78l-.17-.73 1.76-1.01-1-1.74-1.77 1.02-.54-.51a6 6 0 0 0-2.4-1.4l-.72-.2V4h-2v2.04l-.71.2a6 6 0 0 0-2.41 1.4zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm0-2a2 2 0 1 0 0-4 2 2 0 0 0 0 4z"/></svg>
      </div>
      <div class="tool-button" @click="showMenu">
        &vellip;
      </div>
    </div>
  </div>
</template>

<script>
  export default {
    name: "TopLeftHeader",
    props: ["profile", "showContactsMenu", "showOptionsMenu"],
    data() {
      return {
        defaultPhoto: 'https://a.slack-edge.com/66f9/img/avatars-teams/ava_0001-34.png',
      }
    },
    methods: {
      showContacts() {
        this.$emit('show-contacts', !this.showContactsMenu);
      },
      showMenu() {
        this.$emit('show-options', !this.showOptionsMenu);
      },
      openProfile() {
        this.$emit('open-profile');
      },
      showNewContactMenu() {
        this.$emit('show-new-contact-menu');
      },
      showNewGroupMenu() {
        this.$emit('show-new-group-menu');
      },
      showSettings() {
        console.debug('%c Showing settings', 'background: #228B22; color: #ffffff');
        this.$emit('show-settings');
      }
    },
    computed: {
      profileImage() {
        return (this.profile && this.profile.imageUrl) || this.defaultPhoto;
      },
    }
  }
</script>

<style scoped>
.top-left-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  box-sizing: border-box;
  width: 100%;
}

.top-left-header--text{
  color: #fff;
  display: flex;
  font-size: 15px;
  font-weight: 600;
  flex: 1 1 auto;
}

.top-left-header--tools{
  display: flex;
  justify-content: center;
  align-items: center;
  flex: 0 0 auto;
  margin-right: 0.5rem;
}

.tool-button{
  display: flex;
  align-items: center;
  justify-content: center;
  box-sizing: border-box;
  color: #fff;
  background-color: unset;
  padding: 8px;
  outline: none;
  font-size: large;
  width: 40px;
  height: 40px;
  font-weight: bold;
  margin-left: 3px;
}

.tool-button:hover {
  background-color: rgba(0, 0, 0, 0.15);
  border-radius: 50%;
  cursor: pointer;
}

.tool-button:active {
  background-color: rgba(0, 0, 0, 0.20);
  border-radius: 50%;
  cursor: pointer;
}

.profile--img {
  border-radius: 50%;
  align-self: center;
  padding: 10px;
  height: 40px;
  width: 40px;
  flex: 0 0 auto;
  box-sizing: content-box;
}

.tool-button svg path {
  fill: white;
}
</style>